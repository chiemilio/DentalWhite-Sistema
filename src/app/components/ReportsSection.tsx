...
// Datos mock de ingresos por sucursal y mes
const generateIncomeData = () => {
  ...
  return meses.map((mes) => {
    ...
    sucursales.forEach((sucursal) => {
      data[sucursal] = Math.floor(Math.random() * 80000) + 40000;
    });
    data.total = sucursales.reduce((sum, sucursal) => sum + data[sucursal], 0);
    return data;
  });
};

const generateServiceData = (appointments: any[]) => {
  ...
  appointments.forEach((apt) => {
    if (apt.status === 'completed') {
      ...
      const price = servicePrices[apt.serviceName] || 1000;
      serviceIncome[apt.serviceName] = (serviceIncome[apt.serviceName] || 0) + price;
    }
  });
...
const generateServiceDataByBranch = (appointments: any[]) => {
  const branches = ['Pénjamo', 'Valle de Santiago', 'Abasolo'];
  const servicesByBranch: { [key: string]: any } = {};
  ...
  branches.forEach(branch => {
    const branchAppointments = appointments.filter(apt => 
      apt.workCenterName === branch && apt.status === 'completed'
    );
    ...
    servicesByBranch[branch] = Object.entries(serviceCounts).map(([name, value]) => ({
      service: name,
      branch: branch,
      consultas: value,
      ingresos: serviceIncome[name] || 0,
  ...
  // Flatten all services from all branches
  const allServices: any[] = [];
  Object.values(servicesByBranch).forEach((services: any) => {
...
const generateBranchData = (appointments: any[]) => {
  const branchCounts: { [key: string]: number } = {};
  const branchIncome: { [key: string]: number } = {};
  
  appointments.forEach((apt) => {
    if (apt.status === 'completed') {
      const branch = apt.workCenterName;
      branchCounts[branch] = (branchCounts[branch] || 0) + 1;
      // Ingreso aproximado por cita
      branchIncome[branch] = (branchIncome[branch] || 0) + (Math.random() * 10000 + 2000);
    }
  });
...
export function ReportsSection({ appointments, patients, employees }: ReportsSectionProps) {
  ...
  const serviceData = generateServiceData(appointments);
  const branchData = generateBranchData(appointments);
  const serviceDataByBranch = generateServiceDataByBranch(appointments);

  // Filtrar datos por sucursal
  const filteredAppointments = branch === 'all' 
    ? appointments 
    : appointments.filter(apt => apt.workCenterName === branch);

  const filteredServiceData = branch === 'all' 
    ? serviceData 
    : generateServiceData(filteredAppointments);
  
  // Para mostrar datos desglosados por sucursal cuando se selecciona "all"
  const detailedServiceData = branch === 'all' ? serviceDataByBranch : filteredServiceData;

  // Calcular totales
  const totalIncome = filteredServiceData.reduce((sum, item) => sum + item.ingresos, 0);
  const totalAppointments = filteredAppointments.filter(a => a.status === 'completed').length;
  ...
  const handlePrintReport = () => {
    ...
    if (svgElement) {
      ...
      const pdf = new jsPDF();
      pdf.text('Reporte de Consultas y Ingresos', 10, 10);
      pdf.addImage(svgData, 'SVG', 10, 20, 190, 0);
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid md:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col gap-2">
              <Button onClick={handleViewReport} className="bg-purple-500 hover:bg-purple-600">
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={incomeData}>
                ...
                <Tooltip formatter={(value) => `$${value.toLocaleString('es-MX')}`} />
                <Legend />
                <Line type="monotone" dataKey="Centro" stroke="#0ea5e9" strokeWidth={2} />
                <Line type="monotone" dataKey="Norte" stroke="#8b5cf6" strokeWidth={2} />
                <Line type="monotone" dataKey="Sur" stroke="#10b981" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      {reportType === 'appointments' && (
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="border-sky-200">
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={incomeData.map(d => ({ mes: d.mes, consultas: Math.floor(Math.random() * 100) + 50 }))}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="mes" />
                  <YAxis />
                  <Tooltip />
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-sky-50">
                <tr>
                  {reportType === 'services' && branch === 'all' && (
                    <>
              ...
              <tbody>
                {reportType === 'services' && branch === 'all' && detailedServiceData.map((item, index) => (
                ...
                {reportType === 'services' && branch !== 'all' && filteredServiceData.map((item, index) => (
                  <tr key={index} className="border-b border-gray-100">
                ...
                <tr className="bg-sky-50 font-bold">
                  <td className="p-3" colSpan={reportType === 'services' && branch === 'all' ? 2 : 1}>TOTAL</td>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-white p-6 border-2 border-sky-200 rounded-lg space-y-6">
            {/* Header */}
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    ...
                    <tr className="bg-sky-50">
                      <td className="p-3 text-sm" colSpan={reportType === 'services' && branch === 'all' ? 2 : 1}>TOTAL</td>
