...
import { apiClient, BackendPatient } from '../utils/api';
...
export function NewAppointmentDialog({ onAppointmentCreated, existingAppointments }: NewAppointmentDialogProps) {
  ...
  const loadEmployees = async () => {
    try {
      ...
      const data = await apiClient.get<Array<{id: number, usuario_nombre: string}>>('/employees/', true);
...
// Get available services for selected branch - now uses database data
  ...
  const handleScheduleAppointment = async () => {
    ...
    if (patientType === 'existing') {
      // Use selectedPatient from API search results
      ...
      console.log('Creating patient and appointment with:', newPatientDataCombined);
    ...
    console.log('patientId:', patientId, 'selectedEmployeeId:', selectedEmployeeId);
    try {
      const fechaHora = new Date(`${newAppointment.date}T${newAppointment.time}`);
      ...
      console.log('Calling /appointments/ with auth=true');
